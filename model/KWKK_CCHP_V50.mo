package KWKK_CCHP_V50
  package SimModels
    model KWKK_LOOP
      KWKK_CCHP_V50.Components.Switch Switch(AdCM_ON = false, CHP_ON = true, Coil_ON = false, RevHP_CC_ON = false, RevHP_HP_ON = false) annotation(
        Placement(visible = true, transformation(origin = {-60, 80}, extent = {{-20, -20}, {20, 20}}, rotation = 0)));
      Components.CHPUnit CHP(Higher_Temp_Limit = 70, Lower_Temp_Limit = 70) annotation(
      ...
      Components.HTES_Loop HTES_Loop(T_ini_set = 30) annotation(
      ...
      Components.OutdoorCoil_NTU_CCM OC_3_RevHP annotation(
        Placement(visible = true, transformation(origin = {70, 70}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Components.CTES_Loop CTES_Loop(T_ini_set = 20, kappa = 0.001) annotation(
        Placement(visible = true, transformation(origin = {70, -10}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    equation
      ...
      annotation(
        experiment(StartTime = 0, StopTime = 36000, Tolerance = 1e-06, Interval = 60),
  ...
  package Components
    model CHPUnit
      ...
      Units.Power_kW CHP_H_E_PE_M "Real electrical power produced by CHP [kW]";
...
//======================== Fuel consumption [l/h] ========================================
      CHP_H_F_VF_M = (CHP_H_E_PE_M + CHP_H_W_PT_M) * 1000 / ((CHP_eta_Thermal + CHP_eta_Electrical) * CHP_Fuel_HHV * rho_fuel) "[kW] * 1000 / [(kWh/kg) *(kg/m³)] = (l/h) ";
//=========================== Color and Shape ====================================
      annotation(
...
//================== Temperatures from °C to K ====================
      RevHP_HC_W_T_M_MT_FL_K = RevHP_HC_W_T_M_MT_FL_ + 273.15;
      RevHP_HC_W_T_M_LT_FL_K = RevHP_HC_W_T_M_LT_FL_ + 273.15;
      RevHP_HC_W_T_M_MT_RL_K = RevHP_HC_W_T_M_MT_RL_ + 273.15;
      RevHP_HC_W_T_M_LT_RL_K = RevHP_HC_W_T_M_LT_RL_ + 273.15;
...
//==== Calculate RevHP_HC_W_T_M_MT_FL_K using Energy Balance in HT Circuit =====
//    RevHP_HC_W_PT_M_MT__ = m_dot_HT_FL * cpw * (RevHP_HC_W_T_M_MT_FL_K - RevHP_HC_W_T_M_MT_RL_K);
      RevHP_HC_W_T_M_MT_FL_K = RevHP_HC_W_T_M_MT_RL_K + RevHP_HC_W_PT_M_MT__ / (m_dot_HT_FL_Set * cpw);
//==== Calculate RevHP_HC_W_T_M_LT_FL_K using Energy Balance in MT Circuit =====
//    RevHP_HC_W_PT_M_LT__ = m_dot_MT_FL * cpw * (RevHP_HC_W_T_M_LT_RL_K - RevHP_HC_W_T_M_LT_FL_K) "Temp. otherway round because temp. coming back from OC is warmer";
      RevHP_HC_W_T_M_LT_FL_K = RevHP_HC_W_T_M_LT_RL_K - RevHP_HC_W_PT_M_LT__ / (m_dot_MT_FL_Set * cpw);
//================== Power Input =============================
...
//================ Color and shape ==============
    ...
    model HTES_Loop "Hot storage stratified tank (HTES) based on eicker book / paper"
      ...
      parameter SI.Temp_C T_ini_set = 22 "Initial temperature of the tank [°C], Should always be higher than T_AdCM_RL/T_Load_RL and lower then heat hysteresis loop" annotation(
        HideResult = true);
      ...
      parameter Integer Coil_Safety_Layer = 31 "Define layer whose temperature decides COIL Safety" annotation(
        HideResult = true);
      parameter Integer Load_FL_Layer = 61 "Define layer from which water goes to LOAD" annotation(
        HideResult = true);
      parameter Integer AdCM_FL_Layer = 82 "Define layer from which water goes to AdCM" annotation(
        HideResult = true);
      ...
      parameter Integer HX_Layer = 10 "Define Layer from which water goes to the HX" annotation(
        HideResult = true);
      parameter Integer Temp1 = 61 "Layer of the tank which controls the Hysteresis (Lower Temperature Limit)" annotation(
        HideResult = true);
      ...
      HideResult = true);
      //  SI.Temp_C T_ini[n] = linspace(35,T_ini_set, n) "fill an array of size n with values LINEARLY SPACED from defined temperature to T_ini_set" annotation(HideResult = true);
      /********************* Temperatures in K *************************/
...
//Simple forward difference model. Heat flow is calculated using Fourier equation.Solution is substantially simplified if only the energy from the preceding layer is considered. For the first and the last layer the effective mass flow is zero
      ...
      for i in Load_FL_Layer:n - 1 loop
        m_dot[i] = m_dot_CHP + m_dot_RevHP_HT - m_dot_AdCM_HT "effective mass flow for layers between top of tank and layer where load is extracted";
      end for;
      for i in 2:Load_FL_Layer - 1 loop
        m_dot[i] = m_dot[Load_FL_Layer] - m_dot_LOAD "effective mass flow for layers between bottom of tank and layer where load is extracted";
      end for;
      if m_dot[Load_FL_Layer - 1] > 0 then
        d_pos = 1;
      else
        d_pos = 0;
      end if;
//===nth layer at top of the tank has a different equation; Return temp coming from CHP/RevHP.Temp going to Load and AdCM. ==================
// forced convection cooling occurs from preceding layer only when d_pos = 0
      cp * mi * der(HTES_H_W_T_M_IT_K[n]) = Alayer * lambda_eff * (HTES_H_W_T_M_IT_K[n - 1] - HTES_H_W_T_M_IT_K[n]) / zi + cp * m_dot_CHP * (T_HTES_CHP_In_K - HTES_H_W_T_M_IT_K[n]) + cp * m_dot_RevHP_HT * (T_HTES_RevHP_In_K - HTES_H_W_T_M_IT_K[n]) + (1 - d_pos) * cp * m_dot[n - 1] * (HTES_H_W_T_M_IT_K[n] - HTES_H_W_T_M_IT_K[n - 1]) - Aamb * kappa * (HTES_H_W_T_M_IT_K[n] - T_amb_K) + COIL_H_E_PT_M__ / n;
//================== Load_FL_Layer to n-1 th Layer========
      for i in Load_FL_Layer:n - 1 loop
        cp * mi * der(HTES_H_W_T_M_IT_K[i]) = Alayer * lambda_eff * (HTES_H_W_T_M_IT_K[i - 1] - 2 * HTES_H_W_T_M_IT_K[i] + HTES_H_W_T_M_IT_K[i + 1]) / zi + d_pos * cp * m_dot[i] * (HTES_H_W_T_M_IT_K[i + 1] - HTES_H_W_T_M_IT_K[i]) + (1 - d_pos) * cp * m_dot[i] * (HTES_H_W_T_M_IT_K[i] - HTES_H_W_T_M_IT_K[i - 1]) - Aamb * kappa * (HTES_H_W_T_M_IT_K[i] - T_amb_K) + COIL_H_E_PT_M__ / n;
      end for;
//================== 2nd to(Load_FL_Layer-1)th Layer========
      for i in 2:Load_FL_Layer - 1 loop
        cp * mi * der(HTES_H_W_T_M_IT_K[i]) = Alayer * lambda_eff * (HTES_H_W_T_M_IT_K[i - 1] - 2 * HTES_H_W_T_M_IT_K[i] + HTES_H_W_T_M_IT_K[i + 1]) / zi + d_pos * cp * m_dot[i] * (HTES_H_W_T_M_IT_K[i + 1] - HTES_H_W_T_M_IT_K[i]) + (1 - d_pos) * cp * m_dot[i] * (HTES_H_W_T_M_IT_K[i] - HTES_H_W_T_M_IT_K[i - 1]) - Aamb * kappa * (HTES_H_W_T_M_IT_K[i] - T_amb_K) + COIL_H_E_PT_M__ / n;
      end for;
...
//================= Shape and Color =================
    ...
    model CTES_Loop "Cold Storage Tank"
      ...
      SI.Temp_C T_CTES_LOAD_RL "Temperature coming back from the LOAD [°C]" annotation(
        HideResult = true);
      SI.Temp_C T_ini[n] = fill(T_ini_set, n) "fill an array of size n with value T_ini_set that is defined as parameter" annotation(HideResult = true);
    //  SI.Temp_C T_ini[n] = linspace(13, T_ini_set, n) "fill an array of size n with values LINEARLY SPACED between predefined and T_ini_set" annotation(
    //    HideResult = true);
      //********************* Temperatures in K *************************//
...
//
//forced convection heating occurs from preceding layer only when d_pos is 0.
      cp * mi * der(CTES_H_W_T_M_IT_K[1]) = m_dot_AdCM * cp * (T_CTES_AdCM_In_K - CTES_H_W_T_M_IT_K[1]) + cp * m_dot_RevHP * (T_CTES_RevHP_In_K - CTES_H_W_T_M_IT_K[1]) - Aamb * kappa * (CTES_H_W_T_M_IT_K[1] - T_amb_K) + Alayer * lambda_eff * (CTES_H_W_T_M_IT_K[2] - CTES_H_W_T_M_IT_K[1]) / zi + (1 - d_pos) * cp * m_dot[2] * (CTES_H_W_T_M_IT_K[1] - CTES_H_W_T_M_IT_K[2]);
//================== From 2nd to (n-1)th Layer; middle of the tank ========
      for i in 2:n - 1 loop
        cp * mi * der(CTES_H_W_T_M_IT_K[i]) = Alayer * lambda_eff * (CTES_H_W_T_M_IT_K[i - 1] - 2 * CTES_H_W_T_M_IT_K[i] + CTES_H_W_T_M_IT_K[i + 1]) / zi + (1 - d_pos) * cp * m_dot[i] * (CTES_H_W_T_M_IT_K[i] - CTES_H_W_T_M_IT_K[i + 1]) + d_pos * cp * m_dot[i] * (CTES_H_W_T_M_IT_K[i - 1] - CTES_H_W_T_M_IT_K[i]) - Aamb * kappa * (CTES_H_W_T_M_IT_K[i] - T_amb_K);
      end for;
...
//=============== Color and Shape====================
    ...
    model Ambient_Table_T
      ...
      Modelica.Blocks.Sources.CombiTimeTable combiTimeTable(fileName = "Z:\6_Modelle+Validierung\Functional Measurements\BHKW\BHKW_VAL_2017_11_27\Temperature.csv", tableName = "tab1", tableOnFile = true) annotation(
        Placement(visible = true, transformation(origin = {-50, 10}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    equation
...
//=============== Color and Shape====================
    ...
    model Power_Table_kW
      ...
      import SI = Modelica.SIunits;
      //=============== Color and Shape====================
      Modelica.Blocks.Sources.CombiTimeTable combiTimeTable(fileName = "C:\Users\psawant\Documents\Power.csv", smoothness = Modelica.Blocks.Types.Smoothness.ConstantSegments, tableName = "tab1", tableOnFile = true) annotation(
        Placement(visible = true, transformation(origin = {-50, 10}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Interfaces.Realtophys_power Realtophys_power annotation(
        Placement(visible = true, transformation(origin = {30, 10}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      KWKK_CCHP_V50.Interfaces.Power Power1 annotation(
